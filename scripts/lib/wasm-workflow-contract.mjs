...
export function validateWasmWorkflow(source, engineVersionSource) {
  ...
  requireContract(hasExactKeys(workflow.jobs, ["build"]), "jobs must contain only build");
  ...
  requireContract(hasExactKeys(job, ["runs-on", "timeout-minutes", "steps"]), "build job fields must be exact and must not override permissions");
...
export const wasmWorkflowContract = { requiredPaths, artifactPaths, uploadAction, requiredSourceCommit, requiredImage };
