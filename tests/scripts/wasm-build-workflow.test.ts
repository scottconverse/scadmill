...
describe("official-source OpenSCAD WASM build workflow", () => {
  ...
  it.each<[string, string, RegExp]>([
    ...
    ["wrong snapshot version", workflow.replaceAll(wasmWorkflowContract.requiredVersion, "2026.06.13"), /approved snapshot version/],
    ["unbound snapshot version", workflow.replace('-DOPENSCAD_VERSION="$OPENSCAD_VERSION"', "-DOPENSCAD_VERSION=2026.06.12"), /container and build commands must be exact/],
    ["snapshot version omitted from container", workflow.replace("            --env OPENSCAD_VERSION \\\n", ""), /container and build commands must be exact/],
