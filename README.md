...
| C4 web/WASM | The exact official source commit now builds successfully inside the digest-pinned OpenSCAD WASM image. Hosted run 29495964243 produced checksum-verified JavaScript, WASM, and manifest outputs with no unrecorded sidecars. The structured worker service boundary is implemented and independently reviewed at strict zero, including isolated concurrent operations, render-only supersession, targeted timeout/cancel cleanup, output replay, and defensive binary transfer. Q-0023 governs only whether the exact-date official prebuilt artifact may substitute for this completed source-build path. | Integrate the generated module through the real worker adapter and cache/download UI, then prove AC-4.a byte parity for F1/F2 STL and F3 SVG plus the three public examples. |
