...
| C7 formatter | Not started on this branch. | Fifteen Appendix E goldens, idempotence, refusal on syntax errors, and geometry preservation. |
| C13 platform/installers | The Windows path now produces an offline NSIS setup executable with normal per-user uninstall support. The app statically links the Visual C++ runtime, and CI signs the setup through Azure Artifact Signing when owner credentials are available before generating its checksum. A local install → launch → uninstall journey passes. | Hosted signed-artifact evidence, a fresh-Sandbox installer journey, static web engine target, declared platform degradation, single-instance/file associations, and macOS/Linux installers remain. |
