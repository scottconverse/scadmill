...
| C4 web/WASM | A candidate-runnable GitHub workflow now builds from the exact official source commit inside a digest-pinned OpenSCAD WASM image and retains checksummed JavaScript, WASM, and manifest outputs. No WASM artifact has passed that hosted build yet. Q-0023 governs only whether the exact-date official prebuilt artifact may substitute for this mandated source build. | Produce and verify the hosted artifact, integrate the worker-backed browser engine, then prove AC-4.a byte parity for F1/F2 STL and F3 SVG plus the three public examples. |
