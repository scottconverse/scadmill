...
The project is being delivered milestone by milestone from the clean-room functional specification in [`spec/scadmill-spec-v0.6.md`](spec/scadmill-spec-v0.6.md). M0 and M1 are accepted, and the exact M2 candidate passed its local, clean Windows Sandbox, hosted CI, and isolated owner similarity gates. M3 is now underway. Open decisions remain isolated in [`spec/QUESTIONS.md`](spec/QUESTIONS.md) without retroactively invalidating accepted milestones.
...
|---|---|---|
| C14 welcome | The first-run surface, durable suppress-on-launch preference, recent-project access, safe workspace replacement, and all three normative Appendix F samples are implemented. | The final real-engine fresh-profile journey will be repeated through the M3 native/WASM parity gate. FR-15.13 thumbnails remain correctly scheduled for M4. |
| C4 web/WASM | The exact official source commit now builds reproducibly inside the digest-pinned OpenSCAD WASM image: two isolated hosted runs produced byte-identical JavaScript, WASM, and manifest outputs. The structured worker service boundary is implemented and independently reviewed at strict zero. The artifact-independent runtime now mirrors native VFS staging, parameter/preview arguments, binstl-to-SVG render fallback, exports including PNG policy, byte output capture, result parsing, and complete workspace cleanup under 15 focused fake-module tests. Q-0023 governs only whether the exact-date official prebuilt artifact may substitute for this completed source-build path. | Connect the verified generated module and runtime through the real worker and separately fetched asset loader, add cache/download UI, then prove AC-4.a byte parity for F1/F2 STL and F3 SVG plus the three public examples. |
| C7 formatter | The parse-gated formatter produces all fifteen Appendix E goldens exactly and idempotently, preserves comments and Customizer annotations, formats documents and selections through the editor/menu command path, honors configurable indentation and format-on-save, and refuses malformed source without changing it. The four renderable golden pairs produce byte-identical STL output before and after formatting on OpenSCAD 2026.06.12. | C7 implementation and its local acceptance evidence are complete; the exact candidate still passes through the aggregate M3 gate. |
| C13 platform/installers | The Windows path now produces an offline NSIS setup executable with normal per-user uninstall support. The app statically links the Visual C++ runtime, and CI signs the setup through Azure Artifact Signing before generating its checksum. Hosted run 29459417158 passed runtime-import inspection, signing, Authenticode verification, hashing, and upload; the downloaded setup independently reports a valid signature. A local install → launch → uninstall journey also passes. | A fresh-Sandbox installer journey for the signed setup, static web engine target, declared platform degradation, single-instance/file associations, and macOS/Linux installers remain. |
