...
| C6 files/projects/export | Blank scratch mode, functional File commands and shortcuts, discoverable browser Create/Open workspace actions, a typed native folder chooser with advanced manual-path fallback, desktop/web storage, safe file operations, external-change handling, recovery, recent projects, ZIP/share portability, and full-quality exports. | Welcome-screen duplication, autosave-control semantics, and application-owned export-destination picking remain Q-0029/Q-0030/Q-0031. Dirty/final-tab close policy remains Q-0018/Q-0019. |
...
| Manual evidence | Automated behavior and real-browser PNG evidence are retained in the test suites and milestone-gate artifacts. | A live packaged-desktop first-use smoke—including real OS interaction with the newly wired folder chooser—the FR-2.5 2-million-triangle/2020-class-integrated-GPU frame-rate measurement, a live process-kill/relaunch AC-6.d recovery artifact, and a packaged production-OS credential-store round trip are not yet recorded; no completed verification claim is made for them. |
...
`pnpm check:generated`, `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm test:e2e`, `pnpm build`, `pnpm check:provenance`, `pnpm check:licenses all`, `cargo fmt -- --check`, `cargo clippy -- -D warnings`, and the Rust test suite form the local quality gate. Run the retained M2 browser evidence lane with `pnpm.cmd exec playwright test --config tests/e2e/m2-gate.playwright.config.ts`; set `SCADMILL_GATE_ARTIFACT_DIR` to retain its attestations, screenshot, HTML, traces, JSON report, exact runner copies, and SHA-256 hashes outside the repository. Run `pnpm profile:viewer` for the hardware-disclosing two-million-triangle orbit profile. A passing run is acceptance evidence only when `SCADMILL_PERF_HARDWARE_QUALIFICATION=2020-class-integrated` truthfully describes the disclosed host; the retained AMD Radeon 780M result is diagnostic because that GPU is newer. After changing `src/ui/editor/openscad.grammar`, run `pnpm generate:openscad-parser` and commit both generated TypeScript files. Install the acceptance-test browser once with `pnpm exec playwright install chromium`. Run the Rust commands with `--manifest-path` for both manifests under `src/native-engine` and `src/desktop-shell/src-tauri`. Both npm and Rust dependency-license lanes are green after the owner-approved Unicode-3.0 policy addition. The owner-supplied independence gate runs only in isolated GitHub Actions and must never be executed locally.
