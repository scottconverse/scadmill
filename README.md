...
|---|---|---|
| C2/C3 viewers | Native-backed STL and allowlist-sanitized SVG presentation, controlled camera, point-to-point measurement, per-project/file durable annotations, exact bounds, screenshots, 2D routing, and real-browser capture evidence. | Incompatible pinned-mode presentation, edge/face semantics, screenshot overlay composition, and keyboard geometry picking remain Q-0024/Q-0025/Q-0026/Q-0028. |
...
| C6 files/projects/export | Blank scratch mode, functional File commands and shortcuts, desktop/web storage, safe file operations, external-change handling, recovery, recent projects, ZIP/share portability, and full-quality exports. | Welcome-screen duplication, autosave-control semantics, and application-owned destination picking remain Q-0029/Q-0030/Q-0031. Dirty/final-tab close policy remains Q-0018/Q-0019. |
...
| Automated gate evidence | Exact OpenSCAD 2026.06.12 enforcement, TypeScript/lint/build, unit/browser/native/desktop tests, npm-license, generated-parser, and provenance gates. | Rust Unicode-3.0 policy remains Q-0001; the owner similarity gate remains isolated CI-only. |
| Manual evidence | Automated behavior and real-browser PNG evidence are retained in the test suites and milestone-gate artifacts. | A live packaged-desktop first-use smoke, the FR-2.5 2-million-triangle/2020-class-integrated-GPU frame-rate measurement, a live process-kill/relaunch AC-6.d recovery artifact, and a packaged production-OS credential-store round trip are not yet recorded; no completed verification claim is made for them. |
...
`pnpm check:generated`, `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm test:e2e`, `pnpm build`, `pnpm check:provenance`, `pnpm check:licenses npm`, `cargo fmt -- --check`, `cargo clippy -- -D warnings`, and the Rust test suite form the local quality gate. Run the retained M2 browser evidence lane with `pnpm.cmd exec playwright test --config tests/e2e/m2-gate.playwright.config.ts`; set `SCADMILL_GATE_ARTIFACT_DIR` to retain its attestations, screenshot, HTML, traces, JSON report, exact runner copies, and SHA-256 hashes outside the repository. After changing `src/ui/editor/openscad.grammar`, run `pnpm generate:openscad-parser` and commit both generated TypeScript files. Install the acceptance-test browser once with `pnpm exec playwright install chromium`. Run the Rust commands with `--manifest-path` for both manifests under `src/native-engine` and `src/desktop-shell/src-tauri`. The Rust license-policy CI step remains deliberately red on the open Unicode-3.0 policy question Q-0001. The owner-supplied independence gate runs only in isolated GitHub Actions and must never be executed locally.
