...
- **Question:** May the native PNG exporter reject explicit `CameraPose` requests until a pin or amended camera contract provides a lossless mapping for `position`, `target`, and `up`; or should the owner prescribe an engine pin and camera convention that must be used?
- **Evidence:** The pinned official stable OpenSCAD 2021.01 CLI accepts either a six-value eye/center camera or a seven-value translate/rotate/distance camera. Its documented vector form does not support the `up` vector required by Appendix A, and the specification does not define a projection/FOV and Euler-roll conversion for the seven-value form. Passing only eye/center would silently discard valid request data. Sources: [OpenSCAD command-line manual](https://en.wikibooks.org/wiki/OpenSCAD_User_Manual/Using_OpenSCAD_in_a_command_line_environment); [OpenSCAD man page](https://manpages.ubuntu.com/manpages/xenial/man1/openscad.1.html).
