...
- Bound the final M5/M6 release walkthrough to the exact successful CI head, signed installer, normalized packaged-application identity, Windows batch artifacts, update/repair lifecycle, and a clean Windows Sandbox run. The retained record names all 34 M5/M6 capabilities, directly invokes all five installed CLI outcomes, rehashes screenshots on the host, and runs with reliability soak disabled per owner direction. The Windows repair proof now waits for asynchronous scratch restoration before reading the editor instead of racing application startup.
- Closed the beta.3 adversarial release findings: viewer dimensions are now confined to the model surface instead of covering annotation controls; the unconfigured AI panel opens Settings directly on the AI section with focus restoration; Windows, privacy, and decision-ledger copy now matches the shipped engine manager, completed parity proof, updater boundary, and public uninstall state.
- Made release enforcement fail closed. CI now compares provenance against a real base on pull-request, branch-push, tag, and manual paths; missing native/WASM parity evidence is fatal; the Windows setup payload must match the canonical application byte-for-byte after reversing only Tauri's documented `UNK`-to-`NSS` bundle token patch; and the public-beta update, same-version repair, uninstall-state preservation, and reinstall transition are retained as installer evidence.
- Added the missing V-4 production-composition batch-export journey on Windows and Linux. It verifies three real STL downloads and hashes, templated names and geometry, a deliberate item-two engine failure, preservation of item one, continuation to item three, and fail-closed evidence upload.
- Enabled GitHub vulnerability alerts and Dependabot security updates, verified zero open alerts at enablement, and pinned the official dependency-review action so pull requests reject newly introduced high or critical advisories while the existing license gate remains authoritative for license policy.
