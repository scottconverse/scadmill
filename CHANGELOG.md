...
- Hardened packaged AI walkthrough input so WebDriver proves and, at most once, corrects focus on the exact connected enabled Message textarea before low-level keyboard Actions; focus diagnostics contain only fixed-name booleans and never retain message, credential, or active-element attribute values.

- Packaged desktop evidence now enters controlled React textareas through low-level W3C keyboard Actions against the focused element instead of WebDriver's element-value endpoint, releases the virtual keyboard state on every outcome, retains entry and release failures together in causal order, then requires the same consecutive committed-value observations. Exact accessible textarea lookup fails closed on ambiguity; ordinary inputs and selects retain their already-proven bounded setter path.
